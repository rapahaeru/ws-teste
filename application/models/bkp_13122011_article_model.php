 	...
 	function getArticleTopicForMobileBySeoTag($site,$date,$topicMobileTag,$topicForMobileType,$page,$limit){
		...
		if ($topicForMobileType == 'section'){
			...
			if ($topicMobileTag == 'cultura-pop'){
				$w = '';
				$w = $site.'_conteudo.dt_conteudo = "'.$date.'" AND '.$site.'_conteudo.in_ativo = 1 AND '.$site.'_conteudo.in_tipo = 0 AND ('.$site.'_assunto.tx_seo = "'.$topicMobileTag.'" ';
				$w .= ' OR '.$site.'_assunto.tx_seo = "artes"';
